import boto3
import base64
import gzip
import json
import boto3
import ipaddress
import urllib.request

from os import getenv


#mapping used for protocal enrichment
protocal_map = {
  "0": {
    "keyword": "HOPOPT",
    "protocol": "IPv6 Hop-by-Hop Option",
    "reference": "[RFC8200]"
  },
  "1": {
    "keyword": "ICMP",
    "protocol": "Internet Control Message",
    "reference": "[RFC792]"
  },
  "2": {
    "keyword": "IGMP",
    "protocol": "Internet Group Management",
    "reference": "[RFC1112]"
  },
  "3": {
    "keyword": "GGP",
    "protocol": "Gateway-to-Gateway",
    "reference": "[RFC823]"
  },
  "4": {
    "keyword": "IPv4",
    "protocol": "IPv4 encapsulation",
    "reference": "[RFC2003]"
  },
  "5": {
    "keyword": "ST",
    "protocol": "Stream",
    "reference": "[RFC1190][RFC1819]"
  },
  "6": {
    "keyword": "TCP",
    "protocol": "Transmission Control",
    "reference": "[RFC793]"
  },
  "7": {
    "keyword": "CBT",
    "protocol": "CBT",
    "reference": "[Tony_Ballardie]"
  },
  "8": {
    "keyword": "EGP",
    "protocol": "Exterior Gateway protocol",
    "reference": "[RFC888][David_Mills]"
  },
  "9": {
    "keyword": "IGP",
    "protocol": "any private interior gateway             \n(used by Cisco for their IGRP)",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "10": {
    "keyword": "BBN-RCC-MON",
    "protocol": "BBN RCC Monitoring",
    "reference": "[Steve_Chipman]"
  },
  "11": {
    "keyword": "NVP-II",
    "protocol": "Network Voice protocol",
    "reference": "[RFC741][Steve_Casner]"
  },
  "12": {
    "keyword": "PUP",
    "protocol": "PUP",
    "reference":
      "[Boggs, D., J. Shoch, E. Taft, and R. Metcalfe, \"PUP: An\nInternetwork Architecture\", XEROX Palo Alto Research Center,\nCSL-79-10, July 1979; also in IEEE Transactions on\nCommunication, Volume COM-28, Number 4, April 1980.][[XEROX]]"
  },
  "13": {
    "keyword": "ARGUS (deprecated)",
    "protocol": "ARGUS",
    "reference": "[Robert_W_Scheifler]"
  },
  "14": {
    "keyword": "EMCON",
    "protocol": "EMCON",
    "reference": "[<mystery contact>]"
  },
  "15": {
    "keyword": "XNET",
    "protocol": "Cross Net Debugger",
    "reference":
      "[Haverty, J., \"XNET Formats for Internet protocol Version 4\",\nIEN 158, October 1980.][Jack_Haverty]"
  },
  "16": {
    "keyword": "CHAOS",
    "protocol": "Chaos",
    "reference": "[J_Noel_Chiappa]"
  },
  "17": {
    "keyword": "UDP",
    "protocol": "User Datagram",
    "reference": "[RFC768][Jon_Postel]"
  },
  "18": {
    "keyword": "MUX",
    "protocol": "Multiplexing",
    "reference":
      "[Cohen, D. and J. Postel, \"Multiplexing protocol\", IEN 90,\nUSC/Information Sciences Institute, May 1979.][Jon_Postel]"
  },
  "19": {
    "keyword": "DCN-MEAS",
    "protocol": "DCN Measurement Subsystems",
    "reference": "[David_Mills]"
  },
  "20": {
    "keyword": "HMP",
    "protocol": "Host Monitoring",
    "reference": "[RFC869][Bob_Hinden]"
  },
  "21": {
    "keyword": "PRM",
    "protocol": "Packet Radio Measurement",
    "reference": "[Zaw_Sing_Su]"
  },
  "22": {
    "keyword": "XNS-IDP",
    "protocol": "XEROX NS IDP",
    "reference":
      "[\"The Ethernet, A Local Area Network: Data Link Layer and\nPhysical Layer Specification\", AA-K759B-TK, Digital\nEquipment Corporation, Maynard, MA.  Also as: \"The\nEthernet - A Local Area Network\", Version 1.0, Digital\nEquipment Corporation, Intel Corporation, Xerox\nCorporation, September 1980.  And: \"The Ethernet, A Local\nArea Network: Data Link Layer and Physical Layer\nSpecifications\", Digital, Intel and Xerox, November 1982.\nAnd: XEROX, \"The Ethernet, A Local Area Network: Data Link\nLayer and Physical Layer Specification\", X3T51/80-50,\nXerox Corporation, Stamford, CT., October 1980.][[XEROX]]"
  },
  "23": {
    "keyword": "TRUNK-1",
    "protocol": "Trunk-1",
    "reference": "[Barry_Boehm]"
  },
  "24": {
    "keyword": "TRUNK-2",
    "protocol": "Trunk-2",
    "reference": "[Barry_Boehm]"
  },
  "25": {
    "keyword": "LEAF-1",
    "protocol": "Leaf-1",
    "reference": "[Barry_Boehm]"
  },
  "26": {
    "keyword": "LEAF-2",
    "protocol": "Leaf-2",
    "reference": "[Barry_Boehm]"
  },
  "27": {
    "keyword": "RDP",
    "protocol": "Reliable Data protocol",
    "reference": "[RFC908][Bob_Hinden]"
  },
  "28": {
    "keyword": "IRTP",
    "protocol": "Internet Reliable Transaction",
    "reference": "[RFC938][Trudy_Miller]"
  },
  "29": {
    "keyword": "ISO-TP4",
    "protocol": "ISO Transport protocol Class 4",
    "reference": "[RFC905][<mystery contact>]"
  },
  "30": {
    "keyword": "NETBLT",
    "protocol": "Bulk Data Transfer protocol",
    "reference": "[RFC969][David_Clark]"
  },
  "31": {
    "keyword": "MFE-NSP",
    "protocol": "MFE Network Services protocol",
    "reference":
      "[Shuttleworth, B., \"A Documentary of MFENet, a National\nComputer Network\", UCRL-52317, Lawrence Livermore Labs,\nLivermore, California, June 1977.][Barry_Howard]"
  },
  "32": {
    "keyword": "MERIT-INP",
    "protocol": "MERIT Internodal protocol",
    "reference": "[Hans_Werner_Braun]"
  },
  "33": {
    "keyword": "DCCP",
    "protocol": "Datagram Congestion Control protocol",
    "reference": "[RFC4340]"
  },
  "34": {
    "keyword": "3PC",
    "protocol": "Third Party Connect protocol",
    "reference": "[Stuart_A_Friedberg]"
  },
  "35": {
    "keyword": "IDPR",
    "protocol": "Inter-Domain Policy Routing protocol",
    "reference": "[Martha_Steenstrup]"
  },
  "36": {
    "keyword": "XTP",
    "protocol": "XTP",
    "reference": "[Greg_Chesson]"
  },
  "37": {
    "keyword": "DDP",
    "protocol": "Datagram Delivery protocol",
    "reference": "[Wesley_Craig]"
  },
  "38": {
    "keyword": "IDPR-CMTP",
    "protocol": "IDPR Control Message Transport Proto",
    "reference": "[Martha_Steenstrup]"
  },
  "39": {
    "keyword": "TP++",
    "protocol": "TP++ Transport protocol",
    "reference": "[Dirk_Fromhein]"
  },
  "40": {
    "keyword": "IL",
    "protocol": "IL Transport protocol",
    "reference": "[Dave_Presotto]"
  },
  "41": {
    "keyword": "IPv6",
    "protocol": "IPv6 encapsulation",
    "reference": "[RFC2473]"
  },
  "42": {
    "keyword": "SDRP",
    "protocol": "Source Demand Routing protocol",
    "reference": "[Deborah_Estrin]"
  },
  "43": {
    "keyword": "IPv6-Route",
    "protocol": "Routing Header for IPv6",
    "reference": "[Steve_Deering]"
  },
  "44": {
    "keyword": "IPv6-Frag",
    "protocol": "Fragment Header for IPv6",
    "reference": "[Steve_Deering]"
  },
  "45": {
    "keyword": "IDRP",
    "protocol": "Inter-Domain Routing protocol",
    "reference": "[Sue_Hares]"
  },
  "46": {
    "keyword": "RSVP",
    "protocol": "Reservation protocol",
    "reference": "[RFC2205][RFC3209][Bob_Braden]"
  },
  "47": {
    "keyword": "GRE",
    "protocol": "Generic Routing Encapsulation",
    "reference": "[RFC2784][Tony_Li]"
  },
  "48": {
    "keyword": "DSR",
    "protocol": "Dynamic Source Routing protocol",
    "reference": "[RFC4728]"
  },
  "49": {
    "keyword": "BNA",
    "protocol": "BNA",
    "reference": "[Gary Salamon]"
  },
  "50": {
    "keyword": "ESP",
    "protocol": "Encap Security Payload",
    "reference": "[RFC4303]"
  },
  "51": {
    "keyword": "AH",
    "protocol": "Authentication Header",
    "reference": "[RFC4302]"
  },
  "52": {
    "keyword": "I-NLSP",
    "protocol": "Integrated Net Layer Security  TUBA",
    "reference": "[K_Robert_Glenn]"
  },
  "53": {
    "keyword": "SWIPE (deprecated)",
    "protocol": "IP with Encryption",
    "reference": "[John_Ioannidis]"
  },
  "54": {
    "keyword": "NARP",
    "protocol": "NBMA Address Resolution protocol",
    "reference": "[RFC1735]"
  },
  "55": {
    "keyword": "MOBILE",
    "protocol": "IP Mobility",
    "reference": "[Charlie_Perkins]"
  },
  "56": {
    "keyword": "TLSP",
    "protocol": "Transport Layer Security protocol        \nusing Kryptonet key management",
    "reference": "[Christer_Oberg]"
  },
  "57": {
    "keyword": "SKIP",
    "protocol": "SKIP",
    "reference": "[Tom_Markson]"
  },
  "58": {
    "keyword": "IPv6-ICMP",
    "protocol": "ICMP for IPv6",
    "reference": "[RFC8200]"
  },
  "59": {
    "keyword": "IPv6-NoNxt",
    "protocol": "No Next Header for IPv6",
    "reference": "[RFC8200]"
  },
  "60": {
    "keyword": "IPv6-Opts",
    "protocol": "Destination Options for IPv6",
    "reference": "[RFC8200]"
  },
  "61": {
    "keyword": "",
    "protocol": "any host internal protocol",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "62": {
    "keyword": "CFTP",
    "protocol": "CFTP",
    "reference": "[Forsdick, H., \"CFTP\", Network Message, Bolt Beranek and\nNewman, January 1982.][Harry_Forsdick]"
  },
  "63": {
    "keyword": "",
    "protocol": "any local network",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "64": {
    "keyword": "SAT-EXPAK",
    "protocol": "SATNET and Backroom EXPAK",
    "reference": "[Steven_Blumenthal]"
  },
  "65": {
    "keyword": "KRYPTOLAN",
    "protocol": "Kryptolan",
    "reference": "[Paul Liu]"
  },
  "66": {
    "keyword": "RVD",
    "protocol": "MIT Remote Virtual Disk protocol",
    "reference": "[Michael_Greenwald]"
  },
  "67": {
    "keyword": "IPPC",
    "protocol": "Internet Pluribus Packet Core",
    "reference": "[Steven_Blumenthal]"
  },
  "68": {
    "keyword": "",
    "protocol": "any distributed file system",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "69": {
    "keyword": "SAT-MON",
    "protocol": "SATNET Monitoring",
    "reference": "[Steven_Blumenthal]"
  },
  "70": {
    "keyword": "VISA",
    "protocol": "VISA protocol",
    "reference": "[Gene_Tsudik]"
  },
  "71": {
    "keyword": "IPCV",
    "protocol": "Internet Packet Core Utility",
    "reference": "[Steven_Blumenthal]"
  },
  "72": {
    "keyword": "CPNX",
    "protocol": "Computer protocol Network Executive",
    "reference": "[David Mittnacht]"
  },
  "73": {
    "keyword": "CPHB",
    "protocol": "Computer protocol Heart Beat",
    "reference": "[David Mittnacht]"
  },
  "74": {
    "keyword": "WSN",
    "protocol": "Wang Span Network",
    "reference": "[Victor Dafoulas]"
  },
  "75": {
    "keyword": "PVP",
    "protocol": "Packet Video protocol",
    "reference": "[Steve_Casner]"
  },
  "76": {
    "keyword": "BR-SAT-MON",
    "protocol": "Backroom SATNET Monitoring",
    "reference": "[Steven_Blumenthal]"
  },
  "77": {
    "keyword": "SUN-ND",
    "protocol": "SUN ND PROTOCOL-Temporary",
    "reference": "[William_Melohn]"
  },
  "78": {
    "keyword": "WB-MON",
    "protocol": "WIDEBAND Monitoring",
    "reference": "[Steven_Blumenthal]"
  },
  "79": {
    "keyword": "WB-EXPAK",
    "protocol": "WIDEBAND EXPAK",
    "reference": "[Steven_Blumenthal]"
  },
  "80": {
    "keyword": "ISO-IP",
    "protocol": "ISO Internet protocol",
    "reference": "[Marshall_T_Rose]"
  },
  "81": {
    "keyword": "VMTP",
    "protocol": "VMTP",
    "reference": "[Dave_Cheriton]"
  },
  "82": {
    "keyword": "SECURE-VMTP",
    "protocol": "SECURE-VMTP",
    "reference": "[Dave_Cheriton]"
  },
  "83": {
    "keyword": "VINES",
    "protocol": "VINES",
    "reference": "[Brian Horn]"
  },
  "84": {
    "keyword": "IPTM",
    "protocol": "Internet protocol Traffic Manager",
    "reference": "[Jim_Stevens]"
  },
  "85": {
    "keyword": "NSFNET-IGP",
    "protocol": "NSFNET-IGP",
    "reference": "[Hans_Werner_Braun]"
  },
  "86": {
    "keyword": "DGP",
    "protocol": "Dissimilar Gateway protocol",
    "reference":
      "[M/A-COM Government Systems, \"Dissimilar Gateway protocol\nSpecification, Draft Version\", Contract no. CS901145,\nNovember 16, 1987.][Mike_Little]"
  },
  "87": {
    "keyword": "TCF",
    "protocol": "TCF",
    "reference": "[Guillermo_A_Loyola]"
  },
  "88": {
    "keyword": "EIGRP",
    "protocol": "EIGRP",
    "reference": "[RFC7868]"
  },
  "89": {
    "keyword": "OSPFIGP",
    "protocol": "OSPFIGP",
    "reference": "[RFC1583][RFC2328][RFC5340][John_Moy]"
  },
  "90": {
    "keyword": "Sprite-RPC",
    "protocol": "Sprite RPC protocol",
    "reference":
      "[Welch, B., \"The Sprite Remote Procedure Call System\",\nTechnical Report, UCB/Computer Science Dept., 86/302,\nUniversity of California at Berkeley, June 1986.][Bruce Willins]"
  },
  "91": {
    "keyword": "LARP",
    "protocol": "Locus Address Resolution protocol",
    "reference": "[Brian Horn]"
  },
  "92": {
    "keyword": "MTP",
    "protocol": "Multicast Transport protocol",
    "reference": "[Susie_Armstrong]"
  },
  "93": {
    "keyword": "AX.25",
    "protocol": "AX.25 Frames",
    "reference": "[Brian_Kantor]"
  },
  "94": {
    "keyword": "IPIP",
    "protocol": "IP-within-IP Encapsulation protocol",
    "reference": "[John_Ioannidis]"
  },
  "95": {
    "keyword": "MICP (deprecated)",
    "protocol": "Mobile Internetworking Control Pro.",
    "reference": "[John_Ioannidis]"
  },
  "96": {
    "keyword": "SCC-SP",
    "protocol": "Semaphore Communications Sec. Pro.",
    "reference": "[Howard_Hart]"
  },
  "97": {
    "keyword": "ETHERIP",
    "protocol": "Ethernet-within-IP Encapsulation",
    "reference": "[RFC3378]"
  },
  "98": {
    "keyword": "ENCAP",
    "protocol": "Encapsulation Header",
    "reference": "[RFC1241][Robert_Woodburn]"
  },
  "99": {
    "keyword": "",
    "protocol": "any private encryption scheme",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "100": {
    "keyword": "GMTP",
    "protocol": "GMTP",
    "reference": "[[RXB5]]"
  },
  "101": {
    "keyword": "IFMP",
    "protocol": "Ipsilon Flow Management protocol",
    "reference": "[Bob_Hinden][November 1995, 1997.]"
  },
  "102": {
    "keyword": "PNNI",
    "protocol": "PNNI over IP",
    "reference": "[Ross_Callon]"
  },
  "103": {
    "keyword": "PIM",
    "protocol": "protocol Independent Multicast",
    "reference": "[RFC7761][Dino_Farinacci]"
  },
  "104": {
    "keyword": "ARIS",
    "protocol": "ARIS",
    "reference": "[Nancy_Feldman]"
  },
  "105": {
    "keyword": "SCPS",
    "protocol": "SCPS",
    "reference": "[Robert_Durst]"
  },
  "106": {
    "keyword": "QNX",
    "protocol": "QNX",
    "reference": "[Michael_Hunter]"
  },
  "107": {
    "keyword": "A/N",
    "protocol": "Active Networks",
    "reference": "[Bob_Braden]"
  },
  "108": {
    "keyword": "IPComp",
    "protocol": "IP Payload Compression protocol",
    "reference": "[RFC2393]"
  },
  "109": {
    "keyword": "SNP",
    "protocol": "Sitara Networks protocol",
    "reference": "[Manickam_R_Sridhar]"
  },
  "110": {
    "keyword": "Compaq-Peer",
    "protocol": "Compaq Peer protocol",
    "reference": "[Victor_Volpe]"
  },
  "111": {
    "keyword": "IPX-in-IP",
    "protocol": "IPX in IP",
    "reference": "[CJ_Lee]"
  },
  "112": {
    "keyword": "VRRP",
    "protocol": "Virtual Router Redundancy protocol",
    "reference": "[RFC5798]"
  },
  "113": {
    "keyword": "PGM",
    "protocol": "PGM Reliable Transport protocol",
    "reference": "[Tony_Speakman]"
  },
  "114": {
    "keyword": "",
    "protocol": "any 0-hop protocol",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "115": {
    "keyword": "L2TP",
    "protocol": "Layer Two Tunneling protocol",
    "reference": "[RFC3931][Bernard_Aboba]"
  },
  "116": {
    "keyword": "DDX",
    "protocol": "D-II Data Exchange (DDX)",
    "reference": "[John_Worley]"
  },
  "117": {
    "keyword": "IATP",
    "protocol": "Interactive Agent Transfer protocol",
    "reference": "[John_Murphy]"
  },
  "118": {
    "keyword": "STP",
    "protocol": "Schedule Transfer protocol",
    "reference": "[Jean_Michel_Pittet]"
  },
  "119": {
    "keyword": "SRP",
    "protocol": "SpectraLink Radio protocol",
    "reference": "[Mark_Hamilton]"
  },
  "120": {
    "keyword": "UTI",
    "protocol": "UTI",
    "reference": "[Peter_Lothberg]"
  },
  "121": {
    "keyword": "SMP",
    "protocol": "Simple Message protocol",
    "reference": "[Leif_Ekblad]"
  },
  "122": {
    "keyword": "SM (deprecated)",
    "protocol": "Simple Multicast protocol",
    "reference": "[Jon_Crowcroft][draft-perlman-simple-multicast]"
  },
  "123": {
    "keyword": "PTP",
    "protocol": "Performance Transparency protocol",
    "reference": "[Michael_Welzl]"
  },
  "124": {
    "keyword": "ISIS over IPv4",
    "protocol": "",
    "reference": "[Tony_Przygienda]"
  },
  "125": {
    "keyword": "FIRE",
    "protocol": "",
    "reference": "[Criag_Partridge]"
  },
  "126": {
    "keyword": "CRTP",
    "protocol": "Combat Radio Transport protocol",
    "reference": "[Robert_Sautter]"
  },
  "127": {
    "keyword": "CRUDP",
    "protocol": "Combat Radio User Datagram",
    "reference": "[Robert_Sautter]"
  },
  "128": {
    "keyword": "SSCOPMCE",
    "protocol": "",
    "reference": "[Kurt_Waber]"
  },
  "129": {
    "keyword": "IPLT",
    "protocol": "",
    "reference": "[[Hollbach]]"
  },
  "130": {
    "keyword": "SPS",
    "protocol": "Secure Packet Shield",
    "reference": "[Bill_McIntosh]"
  },
  "131": {
    "keyword": "PIPE",
    "protocol": "Private IP Encapsulation within IP",
    "reference": "[Bernhard_Petri]"
  },
  "132": {
    "keyword": "SCTP",
    "protocol": "Stream Control Transmission protocol",
    "reference": "[Randall_R_Stewart]"
  },
  "133": {
    "keyword": "FC",
    "protocol": "Fibre Channel",
    "reference": "[Murali_Rajagopal][RFC6172]"
  },
  "134": {
    "keyword": "RSVP-E2E-IGNORE",
    "protocol": "",
    "reference": "[RFC3175]"
  },
  "135": {
    "keyword": "Mobility Header",
    "protocol": "",
    "reference": "[RFC6275]"
  },
  "136": {
    "keyword": "UDPLite",
    "protocol": "",
    "reference": "[RFC3828]"
  },
  "137": {
    "keyword": "MPLS-in-IP",
    "protocol": "",
    "reference": "[RFC4023]"
  },
  "138": {
    "keyword": "manet",
    "protocol": "MANET protocols",
    "reference": "[RFC5498]"
  },
  "139": {
    "keyword": "HIP",
    "protocol": "Host Identity protocol",
    "reference": "[RFC7401]"
  },
  "140": {
    "keyword": "Shim6",
    "protocol": "Shim6 protocol",
    "reference": "[RFC5533]"
  },
  "141": {
    "keyword": "WESP",
    "protocol": "Wrapped Encapsulating Security Payload",
    "reference": "[RFC5840]"
  },
  "142": {
    "keyword": "ROHC",
    "protocol": "Robust Header Compression",
    "reference": "[RFC5858]"
  },
  "143": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "144": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "145": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "146": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "147": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "148": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "149": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "150": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "151": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "152": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "153": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "154": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "155": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "156": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "157": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "158": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "159": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "160": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "161": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "162": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "163": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "164": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "165": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "166": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "167": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "168": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "169": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "170": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "171": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "172": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "173": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "174": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "175": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "176": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "177": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "178": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "179": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "180": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "181": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "182": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "183": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "184": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "185": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "186": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "187": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "188": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "189": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "190": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "191": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "192": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "193": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "194": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "195": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "196": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "197": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "198": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "199": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "200": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "201": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "202": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "203": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "204": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "205": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "206": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "207": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "208": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "209": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "210": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "211": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "212": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "213": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "214": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "215": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "216": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "217": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "218": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "219": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "220": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "221": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "222": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "223": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "224": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "225": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "226": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "227": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "228": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "229": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "230": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "231": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "232": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "233": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "234": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "235": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "236": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "237": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "238": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "239": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "240": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "241": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "242": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "243": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "244": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "245": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "246": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "247": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "248": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "249": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "250": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "251": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "252": {
    "keyword": "",
    "protocol": "Unassigned",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  },
  "253": {
    "keyword": "",
    "protocol": "Use for experimentation and testing",
    "reference": "[RFC3692]"
  },
  "254": {
    "keyword": "",
    "protocol": "Use for experimentation and testing",
    "reference": "[RFC3692]"
  },
  "255": {
    "keyword": "Reserved",
    "protocol": "",
    "reference": "[Internet_Assigned_Numbers_Authority]"
  }
}
def geoip(ip:str):
    return {}
    #This is not going to scale - need local geoip service
    req = urllib.request.Request("https://ip2c.org/{}".format(ip))
    resp = urllib.request.urlopen(req)
    tmp = str(resp.read()).split(";")
    return {
        "countryCode2":tmp[1],
        "countryCode3":tmp[2],
        "country":tmp[3]
    }

#Main lambda function

def lambda_handler(event, context):
    #init firehose client
    client = boto3.client('firehose')

    #grabbing environment variables
    streamName = getenv("DELIVERYSTREAM")
    project = getenv("PROJECT")

    out = []
    count = 0
    processed = 0
    published = 0
    failed = 0
    parseerror = 0
    for record in event['Records']:
        data = record["kinesis"]["data"]
        uncompressed = gzip.decompress(base64.b64decode(data))
        dat = json.loads(uncompressed)

        #print(dat["logEvents"])
        flowfile = ["version","accountID","interfaceID","srcaddr","dstaddr","srcport","dstport","protocol","packets","bytes","start","end","action","logStatus","vpcID",
        "subnetID","instanceId","tcpFlags","type","pktSrcAddr","pktDstAddr","region","azID","subLocationType","subLocationID","pktSrcAWSService",
        "pktDstAWSService","flowDirection","trafficPath"]
        for rec in dat["logEvents"]:
            processed += 1
            msg = {}
            tmp = rec["message"].split(" ")
            src = None
            dst = None
            try:
              for i in range(0,min(len(flowfile),len(tmp))):
                  if tmp[i] != "-":
                      if i == 10 or i == 11:
                          msg[flowfile[i]] = int(tmp[i])*1000
                      elif i == 7:
                          msg[flowfile[i]] = protocal_map.get(tmp[i])
                          if[msg[flowfile[i]]] is None:
                              msg[flowfile[i]] = {}
                          msg[flowfile[i]]["number"] = tmp[i]
                      elif i == 3:
                          src = ipaddress.ip_address(tmp[i])
                          if not src.is_private:
                              msg[flowfile[i]] = geoip(str(src))
                          else:
                              msg[flowfile[i]] = {}
                          msg[flowfile[i]]["ip"] = tmp[i]
                      elif i == 4:
                          dst = ipaddress.ip_address(tmp[i])
                          if not dst.is_private:
                              msg[flowfile[i]]=geoip(str(dst))
                          else :
                              msg[flowfile[i]] = {}
                          msg[flowfile[i]]["ip"] = tmp[i]

                      else:
                          msg[flowfile[i]] = tmp[i]


                  else:
                      msg[flowfile[i]]= None
            except Exception as err:
              print(err)
              print(tmp)
              parseerror +=1

            if src is not None and dst is not None:
                if src.is_private and dst.is_private:
                    continue



                    resp = urllib.request.urlopen("")




            msg["owner"] = dat["owner"]
            msg["logGroup"] = dat["logGroup"]
            msg["project"] = project
            msg["streamname"] = streamName
            out.append({'Data': json.dumps(msg)})
            published +=1
            count +=1
            if (count>=499):
                result = client.put_record_batch(
                    DeliveryStreamName=streamName,
                    Records=out
                )
                print(out[0])

                print(result)
                out = []
                count = 0
                failed += int(result["FailedPutCount"])
    if count >0:
        print(out[0])
        result = client.put_record_batch(
        DeliveryStreamName=streamName,
        Records=out
        )
        failed += int(result["FailedPutCount"])

        print(result)
    print({"published":published,"processed":processed,"publishfailed":failed,"parsefailed":parseerror})
